...
/* ARM: DISABLE_IRQ saves CPSR, masks IRQ; RESTORE_IRQ restores CPSR */
#define SETEFLAGS(cpsr) asm volatile("msr cpsr, %0" : : "r" (cpsr) : "memory")
#define DISABLE_IRQ    { volatile u4_t oldflags; asm volatile("mrs %0, cpsr" : "=r" (oldflags)); asm volatile("cpsid i" ::: "memory");
#define RESTORE_IRQ    SETEFLAGS(oldflags); }

#define ASSERTCLI
#define ASSERTSTI
#define CLI  asm volatile("cpsid i" ::: "memory");
