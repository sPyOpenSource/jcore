...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *system)
{
    ...
    Status = AllocatePool(EfiLoaderCode, FileSize3, (void**)&ExternalFileBuffer3);
    //Status = SystemTable->BootServices->AllocatePages(AllocateAnyPages, EfiLoaderCode, 1, &ExternalFileBuffer3);
    ...
    char* codefile = (char*)ExternalFileBuffer3 + 11864;//11200;
    ...
    kernel->Read(kernel, &FileSize3, (void *)ExternalFileBuffer3);
...
//__asm__("mov 1049496,%edi");
//__asm__("mov 0x102bc0,%rax");
//__asm__("call *%rax");
